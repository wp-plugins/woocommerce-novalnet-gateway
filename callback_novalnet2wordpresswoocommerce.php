 ...
 * @notice     1. This script must be placed in Wordpress woocommerce shop root folder
...
$payment_method_array = array('novalnet_prepayment', 'novalnet_invoice');
...
/*formatted url
<Site URL>/novalnetpayments/callback_novalnet2wordpresswoocommerce.php?vendor_id=4&status=100&payment_type=INVOICE_CREDIT&tid_payment=12675800001204435&amount=3778&tid=12675800001204435
...
function checkParams($request) {
	...
	if (!in_array($request['payment_type'], $aPaymentTypes)){
		$emailBody .= "Novalnet callback received. But passed payment_type (".$request['payment_type'].") not defined in \$aPaymentTypes: (".implode('; ', $aPaymentTypes).")$lineBreak";
		return false;
	}
