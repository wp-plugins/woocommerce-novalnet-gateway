        ...
        class Novalnet_Payment_Gateway extends WC_Payment_Gateway {
            ...
            public function __construct() {
                ...
                if ( ! empty( $this->global_settings['subs_payments'] ) && in_array( $this->current_payment_id , $this->global_settings['subs_payments'] ) ) {
                    $this->supports = array( 'products', 'subscriptions', 'subscription_cancellation', 'subscription_suspension', 'subscription_reactivation', 'subscription_date_changes' );

                    if ( !in_array( $this->current_payment_id, $this->redirect_payments ) )
                        $this->supports[] = 'subscription_payment_method_change';

                    if ( isset( $_REQUEST['post'] ) && is_admin() ) {
                        $nn_version_check = get_post_meta( $_REQUEST['post'],'_nn_version', true );
                        if ( $nn_version_check ) {
            ...
            /**
             * Gateway rediret response
             *
             * @access public
             * @param none
             * @return void
             */
            function novalnet_redirect_response() {
            ...
            /**
             * Executing Novalnet callback script
             *
             * @access public
             * @param none
             * @return void
             */
            function novalnet_callback_script_execution() {
                if ( isset( $_REQUEST['wc_api'] ) && $_REQUEST['wc_api'] == 'novalnet_callback' ) {
            ...
            public function novalnet_admin_payment_settings() {
                ...
                if ( in_array( $this->current_payment_id, $this->invoice_payments ) ) {
                    ...
                    $this->form_fields['payment_reference_1'] = array(
                    ...
                    $this->form_fields['payment_reference_2'] = array(
                    ...
                    $this->form_fields['payment_reference_3'] = array(
            ...
            public function get_icon() {
                ...
                $icon = ( 'novalnet_cc' == $this->current_payment_id && $this->settings['enable_amex_type'] ) ? NN_Fns()->nn_plugin_url() . '/assets/images/novalnet_cc_amex.png' : $this->payment_details[ $this->current_payment_id ]['payment_logo'];
                $icon_html = ( $this->settings['payment_logo'] ) ? '<img src="' . $icon . '" alt="' . $this->title  . '" title="' . $this->title . '" />' : '';
                $novalnet_logo_html = ($this->settings['novalnet_logo']) ? '<img width="90px" src="' . NN_Fns()->nn_plugin_url() .'/assets/images/novalnet_logo.png" alt="Novalnet AG" title="Novalnet AG" />' : '';

                return apply_filters( 'woocommerce_gateway_icon', $novalnet_logo_html . $icon_html, $this->id);
            }
            ...
            public function payment_fields() {
                if(isset($_SESSION['novalnet']['nn_error_msg'])) {
                    wc_novalnet_display_info( $_SESSION['novalnet']['nn_error_msg'] );
                    unset( $_SESSION['novalnet']['nn_error_msg'] );
                }

                //To avoid multiple payment fields while using woocommerce-german-market plugin
                ...
                if ( in_array( $this->current_payment_id , $this->supports_fraud_check ) ) {
                    ...
                    echo '<input type="hidden" id="nn_vendor"  value="' . $vendor_id . '" /><input type="hidden" id="nn_auth_code" value="' . $vendor_authcode . '" />';
            ...
            public function validate_fields() {
                ...
                if ( isset( $_SESSION['novalnet'][ $this->current_payment_id ]['tid'] ) ) {
                    ...
                    if ( $this->current_payment_id == 'novalnet_cc' ) {
                        $_SESSION['novalnet_cc']['nn_cc_hash'] = !empty( $_POST['nn_cc_hash'] ) ? $_POST['nn_cc_hash'] : $_SESSION['novalnet_cc']['nn_cc_hash'];

                        $_SESSION['novalnet_cc']['novalnet_cc_cvc'] = !empty( $_POST['novalnet_cc_cvc'] ) ? $_POST['novalnet_cc_cvc'] : $_SESSION['novalnet_cc']['novalnet_cc_cvc'];

                        $_SESSION['novalnet_cc']['nn_cc_uniqueid'] = !empty( $_POST['nn_cc_uniqueid'] ) ? $_POST['nn_cc_uniqueid'] : $_SESSION['novalnet_cc']['nn_cc_uniqueid'];
                        if ( ( empty( $_SESSION['novalnet_cc']['novalnet_cc_cvc'] ) || empty( $_SESSION['novalnet_cc']['nn_cc_hash'] ) || empty( $_SESSION['novalnet_cc']['nn_cc_uniqueid']  ) ) ) {
                            $error = __( 'Your credit card details are invalid', 'wc-novalnet' );
                        }
                    } else if ( $this->current_payment_id == 'novalnet_sepa' ) {
                        $_SESSION['novalnet_sepa']['novalnet_sepa_account_holder'] = !empty( $_POST['novalnet_sepa_account_holder'] ) ? $_POST['novalnet_sepa_account_holder'] : $_SESSION['novalnet_sepa']['novalnet_sepa_account_holder'];

                        $_SESSION['novalnet_sepa']['nn_sepa_uniqueid'] = !empty( $_POST['nn_sepa_uniqueid'] ) ? $_POST['nn_sepa_uniqueid'] : $_SESSION['novalnet_sepa']['nn_sepa_uniqueid'];

                        $_SESSION['novalnet_sepa']['nn_sepa_hash'] = !empty( $_POST['nn_sepa_hash'] ) ? $_POST['nn_sepa_hash'] : $_SESSION['novalnet_sepa']['nn_sepa_hash'];
                        if ( empty( $_SESSION['novalnet_sepa']['novalnet_sepa_account_holder'] ) ||  empty( $_SESSION['novalnet_sepa']['nn_sepa_hash'] ) || empty( $_SESSION['novalnet_sepa']['nn_sepa_uniqueid'] ) ) {
                    ...
                    if ( wc_novalnet_check_3d_secure( $this->current_payment_id, $this->settings ) && ! empty( $this->settings['pin_by_callback'] ) &&  isset(  $_SESSION['novalnet'][ $this->current_payment_id . '_pin_by_' . $this->settings['pin_by_callback'] ] ) && wc_novalnet_fraud_prevention_option( $this->settings ) ) {
                        ...
                        $_pin_by_callback =  trim( $_SESSION['novalnet'][ $this->current_payment_id . '_pin_by_' . $this->settings['pin_by_callback'] ] );

                        $condition_error = ( $this->settings['pin_by_callback'] == 'email' ) ? is_email( $_pin_by_callback ) : wc_novalnet_digits_check( $_pin_by_callback );
                        if ( !$condition_error ) {
                            if(isset( $_SESSION[ $this->current_payment_id ] ) ) { unset($_SESSION[ $this->current_payment_id ]); }
                    ...
                    if ( $error ) {
                        ...
                        if ( 'novalnet_cc' == $this->current_payment_id ) {

                            $_SESSION['novalnet'][ $this->current_payment_id ]['pseudo_hash'] = ( $this->global_settings['auto_refill'] ) ? $_SESSION['novalnet_cc']['nn_cc_hash'] : '';
                            $this->card_details = array(
                                'cvc'       => $_SESSION['novalnet_cc']['novalnet_cc_cvc'],
                                'panhash'   => $_SESSION['novalnet_cc']['nn_cc_hash'],
                                'uniqid'    => $_SESSION['novalnet_cc']['nn_cc_uniqueid'],
                            );
                            if ( $this->settings['cc_secure_enabled'] ) {
                                WC()->session->set( $this->current_payment_id, $this->card_details);
                            }
                        } else if ( 'novalnet_sepa' == $this->current_payment_id ) {
                            $auto_refill_enabled = ( $this->global_settings['auto_refill'] || $this->settings['last_succ_refill'] );

                            $_SESSION['novalnet'][ $this->current_payment_id ]['pseudo_hash'] = ( $auto_refill_enabled ) ? $_SESSION['novalnet_sepa']['nn_sepa_hash'] : '';

                            $this->sepa_details = array(
                                'sepa_holder' => $_SESSION['novalnet_sepa']['novalnet_sepa_account_holder'],
                                'sepa_panhash'=> $_SESSION['novalnet_sepa']['nn_sepa_hash'],
                                'sepa_uniqid' => $_SESSION['novalnet_sepa']['nn_sepa_uniqueid']
                            );
                        }
            ...
            public function generate_payment_parameters( $order_id ) {
                ...
                $customer_fields = $this->check_customer_details( $wc_order );
                if( is_array( $customer_fields ) ) {
                    ...
                    $_SESSION['novalnet']['nn_error_msg'] = $customer_fields;
                    return wc_novalnet_redirect();
                }
                ...
                if ( !$is_redirect_method && in_array( $this->current_payment_id, $this->supports_fraud_check ) && wc_novalnet_fraud_prevention_option( $this->settings ) ) {
                    ...
                    $this->payment_parameters[ $this->settings['pin_by_callback'] ] = trim( $_SESSION['novalnet'][ $this->current_payment_id . '_pin_by_' . $this->settings['pin_by_callback'] ]) ;
                }
                ...
                if ( ! empty( $this->settings['reference1'] ) ) {
                    ...
                    $this->payment_parameters['inputval3']= trim( strip_tags( $this->settings['reference1'] ) );
                }
                if ( ! empty( $this->settings['reference2'] ) ) {
                    $this->payment_parameters['input4']   = 'Reference2';
                    $this->payment_parameters['inputval4']= trim( strip_tags( $this->settings['reference2'] ) );
                }
            }
            ...
            public function perform_fraudcheck_pinstatus_call( $order_id ) {
                ...
                $_SESSION['novalnet'][ $this->current_payment_id . '_pin'] = !empty( $_POST[ $this->current_payment_id . '_pin'] ) ? $_POST[ $this->current_payment_id . '_pin'] : (isset($_SESSION['novalnet'][ $this->current_payment_id . '_pin']) ? $_SESSION['novalnet'][ $this->current_payment_id . '_pin'] : '');

                $request_type = ( !empty( $_SESSION['novalnet'][ $this->current_payment_id . '_new_pin'] ) ) ? 'TRANSMIT_PIN_AGAIN' : ( $this->settings['pin_by_callback'] == 'email' ? 'REPLY_EMAIL_STATUS' : 'PIN_STATUS' );

                if(isset($_SESSION['novalnet'][ $this->current_payment_id . '_new_pin'])) { unset($_SESSION['novalnet'][ $this->current_payment_id . '_new_pin']); }
                $this->currency_format( $order->order_total );
                if ( $_SESSION['novalnet'][ $this->current_payment_id ]['amount'] != $this->amount ) {
                    ...
                    $_SESSION['novalnet']['nn_error_msg'] = $error;
                    return wc_novalnet_redirect();
                }
                ...
                if ( $request_type == 'PIN_STATUS' ) {
                    $pin_second_call['pin'] = trim( $_SESSION['novalnet'][ $this->current_payment_id . '_pin'] );
                    unset( $_SESSION['novalnet'][ $this->current_payment_id . '_pin'] );
                }
                ...
                if ( isset( $data['status'] ) && $data['status'] == '100' ) {
                    ...
                    if( $type == 'error' ) {
                        $_SESSION['novalnet']['nn_error_msg'] = $response_text;
                    }else{
            ...
            public function prepare_payment_comments( $response, $test_mode_message, $order ) {
                ...
                if ( ! isset( $_REQUEST['change_payment_method'] ) && in_array( $this->current_payment_id, $this->invoice_payments ) && isset( $response['invoice_iban'] ) ) {
                    ...
                    $novalnet_comments .= __( 'Amount : ', 'wc-novalnet' ) . trim( strip_tags( $order->get_formatted_order_total() ) ) . $new_line;
                    $reference_comments = '';
                    $references = array( $this->settings['payment_reference_1'], $this->settings['payment_reference_2'], $this->settings['payment_reference_3'] );
                    $ac_refernce = array_count_values( $references );
                    $i=1;
                    if ( 'yes' == $references[0] ) {
                        $reference_comments .= ( $ac_refernce['yes'] == 1 )? __( 'Payment Reference : ', 'wc-novalnet' ).$invoice_ref . $new_line : str_replace( '@p',$i, __( 'Payment Reference @p : ', 'wc-novalnet' ) ).$invoice_ref . $new_line;
                        $i++;
                    }
                    if ( 'yes' == $references[1] ) {
                        $reference_comments .= ( $ac_refernce['yes'] == 1 )? __( 'Payment Reference : ', 'wc-novalnet' ).'TID '.$response['tid'].PHP_EOL : str_replace( '@p',$i, __( 'Payment Reference @p : ', 'wc-novalnet' ) ). 'TID '.$response['tid'].$new_line;
                        $i++;
                    }
                    if ( 'yes' == $references[2] ) {
                        $reference_comments .= ( $ac_refernce['yes'] == 1 )? __( 'Payment Reference : ', 'wc-novalnet' ).__( 'Order number : ', 'wc-novalnet' ) . $response['order_no'].$new_line : str_replace( '@p',$i, __( 'Payment Reference @p : ', 'wc-novalnet' )).__( 'Order number ', 'wc-novalnet' ) . $response['order_no'].$new_line;
                    }
                    if( 1 == $ac_refernce['yes'] ) {
                    ...
                    $novalnet_comments = $novalnet_comments . $reference_comments.$new_line.$new_line;
                    if ( $wpdb->get_var( "SHOW COLUMNS FROM `{$wpdb->prefix}novalnet_invoice_details` LIKE 'account_holder';" ) ) {
                        if( !$wpdb->get_var( "SHOW COLUMNS FROM `{$wpdb->prefix}novalnet_invoice_details` LIKE 'payment_reference_1';" ) ) {
                            $wpdb->query( "ALTER TABLE {$wpdb->prefix}novalnet_invoice_details ADD `response_order_no` text DEFAULT NULL, ADD `payment_reference_1` char(5) DEFAULT NULL, ADD `payment_reference_2` char(5) DEFAULT NULL, ADD `payment_reference_3` char(5) DEFAULT NULL, change due_date invoice_due_date  date DEFAULT NULL;");
                        }
                        $wpdb->insert(
                            "{$wpdb->prefix}novalnet_invoice_details",
                            array(
                                'post_id'             => $response['inputval1'],
                                'order_no'            => $order->id,
                                'tid'                 => $response['tid'],
                                ...
                                'payment_reference_1' => $this->settings['payment_reference_1'],
                                'payment_reference_2' => $this->settings['payment_reference_2'],
                                'payment_reference_3' => $this->settings['payment_reference_3']
                            )
                        );
                    }else{
            ...
            public function transaction_success( $response, $message, $order_id ) {
                ...
                $wpdb->insert( "{$wpdb->prefix}novalnet_transaction_detail", array(
                    ...
                    'amount'            => $this->amount,
                    'callback_amount'   => ( ! in_array( $paymentmethod, $this->invoice_payments ) ) ? $this->amount : 0,
                    'currency'          => get_woocommerce_currency(),
                ...
                if ( 'novalnet_paypal' == $paymentmethod && $gateway_status['status'] == 100 ) {
                    $wpdb->insert(
                ...
                if ( ! empty( $gateway_status['subs_id'] ) ) {
                    ...
                    $subscription_length = class_exists( 'WC_Subscriptions_Order' ) ? WC_Subscriptions_Order::get_subscription_length( $order ) : '';
                    $tmp_length = 0;

                    if ( $subscription_length > 0 ){
                        $subscription_freetrial_length = class_exists( 'WC_Subscriptions_Order' ) ? WC_Subscriptions_Order::get_subscription_trial_length( $order ) : '';
            ...
            public function transaction_failure( $response, $message, $order_id ) {
                ...
                if ( ! empty( $response['tid'] ) ) {
                    $response_test_mode = isset( $response['test_mode'] ) ? $response['test_mode'] : '';
                    $test_mode_value = wc_novalnet_check_test_mode_status( $response_test_mode, $this->settings['test_mode'] );
                ...
                $_SESSION['novalnet']['nn_error_msg'] = html_entity_decode( $message, ENT_QUOTES, 'UTF-8' );
            ...
            public function receipt_page( $order_id ) {
                global $wpdb;
                $order_status = $wpdb->get_var( $wpdb->prepare( "SELECT status FROM {$wpdb->prefix}novalnet_transaction_detail WHERE order_no=%d", $order_id ) );
                if ( ! isset( WC()->session->novalnet_receipt_page ) && ( empty( $order_status ) || 100 != $order_status ) ) {
                    ...
                    if(!empty( $order_status ) && !wc_has_notice( __( 'Novalnet Transaction for the Order has been executed / cancelled already.', 'wc-novalnet' ), 'error' ))
                        wc_novalnet_display_info(__( 'Novalnet Transaction for the Order has been executed / cancelled already.', 'wc-novalnet' ));
                }
            }
            ...
            public function check_novalnet_payment_response( $api_response ) {
                if ( isset( $api_response['hash'] ) && ! wc_novalnet_check_hash( $api_response ) ) {
                    $_SESSION['novalnet']['nn_error_msg'] = __( 'While redirecting some data has been changed. The hash check failed', 'wc-novalnet' );
                    wp_safe_redirect( WC()->cart->get_checkout_url() );
                    exit();
                } else {
