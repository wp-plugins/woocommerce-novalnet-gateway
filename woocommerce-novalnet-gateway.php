...
function init_gateway_novalnet() {
    ...
    if (in_array('woocommerce/woocommerce.php', (array) get_option('active_plugins')) || in_array('woocommerce/woocommerce.php', (array) nn_active_nw_plugins())) {
        ...
        if (!defined('NOVALNET_GATEWAY_VERSION'))
            define('NOVALNET_GATEWAY_VERSION','1.1.7');
        ...
        if (!class_exists('WC_Gateway_Novalnet')) {
            ...
            class WC_Gateway_Novalnet extends WC_Payment_Gateway {
                ...
                public function telephone_secondcall($order_id) {
                    ...
                    if (isset($this->vendor_id) && $this->is_digits($this->vendor_id) && $this->vendor_id != null && isset($this->auth_code) && $this->auth_code != null && isset($_SESSION['tel']['novalnet_tel_tid']) && $_SESSION['tel']['novalnet_tel_tid'] != null && isset($this->language) && $this->language != null) {
                ...
                public function generate_additional_parameters($order) {
                    ...
                    if (in_array($this->novalnet_payment_method, $this->invoice_payments)) {
                        $invoice_type = strtoupper(substr($this->novalnet_payment_method, strpos($this->novalnet_payment_method, '_') + 1, strlen($this->novalnet_payment_method)));
                        $invoice_ref = "BNR-" . $this->product_id . "-" . ltrim($order->get_order_number(), _x( '#', 'hash before order number', 'woocommerce' ));
                        $this->payment_parameters['invoice_type'] = $invoice_type;
                ...
                public function prepare_to_novalnet_payport($order) {
                    ...
                    if (!isset($_SESSION['novalnet_receipt_page_got'])) {
                        ...
                        echo '<p>' . __('Thank you for your order, please click the button below to pay with Novalnet.', $this->get_textdomain()) . '</p>';
                ...
                public function backend_data_validation_from_frontend() {
                    ...
                    if ( !$this->vendor_id || !$this->is_digits($this->vendor_id)
						|| !$this->product_id || !$this->is_digits($this->product_id)
						|| !$this->tariff_id || !$this->is_digits($this->tariff_id) || !$this->auth_code
						|| ( (in_array($this->novalnet_payment_method, $this->redirect_payments) || ($this->novalnet_payment_method == 'novalnet_cc' ) && $this->cc3d_enabled == 'yes') && !trim($this->key_password))
						|| ($this->novalnet_payment_method == 'novalnet_paypal'
                ...
                public function shop_backend_validation($request) {
                    ...
                    $vendor_id = isset($request['woocommerce_' . $this->novalnet_payment_method . '_merchant_id']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_merchant_id']) : null;
                    $auth_code = isset($request['woocommerce_' . $this->novalnet_payment_method . '_auth_code']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_auth_code']) : null;
                    $product_id = isset($request['woocommerce_' . $this->novalnet_payment_method . '_product_id']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_product_id']) : null;
                    $tariff_id = isset($request['woocommerce_' . $this->novalnet_payment_method . '_tariff_id']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_tariff_id']) : null;
                    $payment_duration = isset($request['woocommerce_' . $this->novalnet_payment_method . '_payment_duration']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_payment_duration']) : null;
                    $key_password = isset($request['woocommerce_' . $this->novalnet_payment_method . '_key_password']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_key_password']) : null;
                    $api_username = isset($request['woocommerce_' . $this->novalnet_payment_method . '_api_username']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_api_username']) : null;
                    $api_password = isset($request['woocommerce_' . $this->novalnet_payment_method . '_api_password']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_api_password']) : null;
                    $api_signature = isset($request['woocommerce_' . $this->novalnet_payment_method . '_api_signature']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_api_signature']) : null;
                    $manual_check_limit = isset($request['woocommerce_' . $this->novalnet_payment_method . '_manual_check_limit']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_manual_check_limit']) : null;
                    $product_id_2 = isset($request['woocommerce_' . $this->novalnet_payment_method . '_product_id_2']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_product_id_2']) : null;
                    $tariff_id_2 = isset($request['woocommerce_' . $this->novalnet_payment_method . '_tariff_id_2']) ? trim($request['woocommerce_' . $this->novalnet_payment_method . '_tariff_id_2']) : null;
                    $sepa_due_date = isset($request['woocommerce_'. $this->novalnet_payment_method . '_sepa_due_date']) ? trim($request['woocommerce_'. $this->novalnet_payment_method . '_sepa_due_date']) : null;
                    $referrer_id = isset($request['woocommerce_'. $this->novalnet_payment_method . '_referrer_id']) ? trim($request['woocommerce_'. $this->novalnet_payment_method . '_referrer_id']) : null;
                    ...
                    if (isset($manual_check_limit) && $this->is_digits($manual_check_limit) && intval($manual_check_limit) > 0) {
                ...
                public function make_payment_details_array() {
					$this->payment_details = array(
                        ...
                        'novalnet_banktransfer' => array(
                            ...
                            'payment_logo' => LOGO_PATH . 'instant_en_logo.png'
                        ),
                        ...
                        'novalnet_cc' => array(
                            ...
                            'payment_logo' => LOGO_PATH . 'creditcard_logo.jpg'  ,
                        ),
                        ...
                        'novalnet_invoice' => array(
                            ...
                            'payment_logo' =>  LOGO_PATH .'invoice_logo.jpg'
                        ),
                        ...
                        'novalnet_prepayment' => array(
                            ...
                            'payment_description' => 'The bank details will be emailed to you soon after the completion of checkout process.',
                            'payment_logo' => LOGO_PATH .'prepayment_logo.jpg'
                        ),
                        ...
                        'novalnet_sepa' => array(
                            ...
                            'payment_logo' => LOGO_PATH . 'sepa_en_logo.jpg'
                        ),
                    );
                ...
                public function do_novalnet_success($request, $message) {
                    ...
                    if (in_array($this->novalnet_payment_method, $this->invoice_payments)) {
                        ...
                        $novalnet_comments.= __('IBAN', $this->get_textdomain()) . " : " . $request['invoice_iban'] . $new_line;
                        $novalnet_comments.= __('BIC', $this->get_textdomain()) . " : " . $request['invoice_bic'] . $new_line;
                        $novalnet_comments.= __('Bank', $this->get_textdomain()) . " : " . $request['invoice_bankname'] . " " . trim($request['invoice_bankplace']) . $new_line;
                ...
                public function check_novalnetstatus($request, $nn_order_no) {
                    if (isset($request['status']) && $request['status'] == 100 || (isset($request['inputval2']) && $request['inputval2'] == 'novalnet_paypal' && $request['status'] == 90 ))
        ...
        protected function transaction_status($request) {
            ...
            return ($nn_status_code);
				...
				public function novalnet_transaction_meta_boxes() {
					...
					if(in_array($this->order->payment_method,$this->novalnet_extensions)){
				...
				public function novalnet_gateway_transaction_meta_box() {
					...
					<?php
					if ((in_array($this->order->payment_method, $this->novalnet_extensions) && ( get_post_meta($this->order->id,'_nn_capture_code', true) == 1 && get_post_meta($this->order->id,'_nn_cancel_code', true) == 0 )) && $this->order->order_total > 0 )
					:?>
					<ul>
					...
					if ($return == true) {
						...
						if($aryCaptResponse['status'] == NOVALNET_COMPLETE_STATUS){
							...
							$this->display_admin_messages($nn_capt_message,$arr_params);
								...
								$arr_params = remove_query_arg('novalnet_gateway_action');
                    ...
                    if ($this->payment_logo) {
						...
						switch($this->novalnet_payment_method) {
							...
							case 'novalnet_prepayment':
								$custom_width_height = 'width="50px" height="22px"';
								break;
							case 'novalnet_banktransfer':
								$custom_width_height = 'width="60px" height="22px"';
								if($this->language == 'DE')
									$this->icon = LOGO_PATH . 'instant_de_logo.jpg' ;
								break;
							case 'novalnet_cc':
								$custom_width_height = 'width="65px" height="22px"';
								if($this->amex_enabled == 'yes')
									$this->icon = LOGO_PATH . 'creditcard_amex_logo.jpg' ;
								break;
							case 'novalnet_ideal':
								$custom_width_height = 'width="32px" height="22px"';
								break;
							case 'novalnet_paypal':
								$custom_width_height = 'width="82px" height="22px"';
								break;
							case 'novalnet_sepa':
								$custom_width_height = 'width="75px" height="22px"';
								if($this->language == 'DE')
									$this->icon = LOGO_PATH . 'sepa_de_logo.jpg' ;
								break;
							case 'novalnet_tel':
								$custom_width_height = 'width="30px" height="22px"';
								break;
						}
                ...
                public function admin_options() {
					...
					switch($this->novalnet_payment_method) {
						case 'novalnet_cc':
							if($this->settings['amex_enabled'] == 'yes')
								$this->icon = LOGO_PATH . 'creditcard_amex_logo.jpg' ;
							break;
