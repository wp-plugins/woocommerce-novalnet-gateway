                    ...
                    if ( $novalnet_params['subs_billing'] == 1 ) {
                        ...
                        if ( $callback_paid_amount < (int)$novalnet_order_info['amount'] ) {
                            ...
                            if (  $sum_amount >= $novalnet_order_info['amount'] ) {
                                ...
                                if ( $sum_amount > $novalnet_order_info['amount'] )
                                    $callback_comments .= 'Customer paid amount is greater than order amount.';
 ...
 class Novalnet_Callback_handler {
    ...
    public function get_order_reference() {
        ...
        if ( ! empty( $woo_order_id ) ) {
            ...
            $tmp_post_id = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta where meta_value='%s' AND (meta_key='_order_number_formatted' OR meta_key='_order_number')", $woo_order_id ) );
        ...
        if ( $order_details_exists ) {
            $trans_details['sequence_order_no'] = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM $wpdb->postmeta where post_id=%s AND (meta_key='_order_number' OR meta_key='_order_number_formatted')", $trans_details['order_no'] ) );
            ...
            if ( $org_tid != $trans_details['tid'] ) {
                $this->debug_error( 'Novalnet callback received. TID [' . $org_tid . '] is not valid.' );
            }
    ...
    public function perform_subscription_cancel( $post_id , $tid ) {
        ...
        if ( ! empty( $result_set['vendor_id'] ) && ! empty( $result_set['auth_code'] ) && ! empty( $result_set['product_id'] ) && ! empty( $result_set['tariff_id'] ) && ! empty( $tid ) ) {
            ...
            if( 100 == $cancel_response['status'] ) {
                ...
                $subscription_details = $this->get_subscription_details( $post_id );
    ...
    public function send_notification_mail( $datas = array() ) {
        ...
        if ( $receipient_addr && $email_enabled ) {
    ...
    public function recurring_order_creation($subscription_key, $parent_order, $order_info , $parent_next_cycle ) {
        ...
        if ( $this->woo_capture_params['payment_type'] == 'INVOICE_START' ) {
           ...
           $novalnet_comments .= $new_line . __('Please transfer the amount to the below mentioned account details of our payment processor Novalnet', 'wc-novalnet') . $new_line;
            ...
            $novalnet_comments .= __('Amount : ', 'wc-novalnet') . strip_tags( html_entity_decode( woocommerce_price( sprintf( "%0.2f", $this->woo_capture_params['amount']/100) ) ) ). $new_line;
