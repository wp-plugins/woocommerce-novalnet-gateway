<?php
/**
 * Invoice Payment Gateway by Novalnet
 *
 ...
 *
 * This gateway is used for real time processing of Invoice data of customers.
 *
 * @class 		WC_Gateway_Novalnet_Invoice
 * @extends		WC_Novalnet_Payment_Gateway
 * @version		2.0.0
 * @package		woocommerce-novalnet-gateway/includes/gateways/novalnet_invoice
 * @author 		Novalnet
...
/*** Install INVOICE payment to Novalnet Payment Gateway ***/
...
function init_gateway_novalnet_invoice() {
    ...
    class WC_Gateway_Novalnet_Invoice extends WC_Novalnet_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->init_settings();
			$this->method_title 		= 'Novalnet ' . __( 'Invoice', 'novalnet' );
			...
			$this->icon 				= NOVALNET_URL .'assets/images/novalnet_invoice.png';
			$this->has_fields 			= false;

			if ( isset( $this->payment_duration ) && $GLOBALS[ NN_FUNCS ]->is_digits( $this->payment_duration ) ) {
			...
			if(isset($_SESSION['novalnet']['novalnet_invoice']['invoice_info_got']))
				unset($_SESSION['novalnet']['novalnet_invoice']['invoice_info_got']);
			...
			$this->form_fields = array(
				...
				'test_mode' 	=> array(
				 ...
				 'payment_duration' => array(
					'title' 	 => __( 'Payment period in days', 'novalnet' ),
					'type' 		 => 'text',
					'label' 	 => '',
					'default' 	 => '',
					'description'=> __( 'Enter the due date in days, it should be greater than 6. If you leave as empty or less than 7 means default value will be considered as 14 days.', 'novalnet' )
                ),
				'end_user_info' => array(
		...
		public function novalnet_invoice_transactional_info($order) {
			if (!isset($_SESSION['novalnet']['novalnet_invoice']['invoice_info_got']) && $order->payment_method == $this->id) {
				echo wpautop('<h2>' . __('Transaction Information', 'novalnet') . '</h2>');
				echo wpautop(wptexturize($order->customer_note));
				$_SESSION['novalnet']['novalnet_invoice']['invoice_info_got'] = 1;
			}
		}	// End novalnet_invoice_transactional_info()
	}   // End class novalnet_invoice

	$obj = new WC_Gateway_Novalnet_Invoice();
