<?php
/**
 * PayPal Payment Gateway by Novalnet
 *
 ...
 *
 * This gateway is used for real time processing of transaction of customers.
 *
 * @class 		WC_Gateway_Novalnet_Paypal
 * @extends		WC_Novalnet_Payment_Gateway
 * @version		2.0.0
 * @package		woocommerce-novalnet-gateway/includes/gateways/novalnet_paypal
 * @author 		Novalnet
...
/*** Install PAYPAL payment to Novalnet Payment Gateway ***/
...
function init_gateway_novalnet_paypal() {
    ...
    class WC_Gateway_Novalnet_Paypal extends WC_Novalnet_Payment_Gateway {

		var $id 			= NOVALNET_PP;
		var $payment_key 	= PAYMENT_KEY_PP;
		...
		public function __construct() {
			...
			$this->init_settings();
			$this->method_title = 'Novalnet ' . __( 'PayPal', 'novalnet' );
			...
			$this->paypal_pending_status = $this->settings['paypal_pending_status'];
			$this->set_order_status = $this->settings['set_order_status'];
			$this->icon 		= NOVALNET_URL .'assets/images/novalnet_paypal.png';
			...
			add_action('init', array(&$this, 'check_paypal_response'));
			if(isset($_SESSION['novalnet']['novalnet_paypal_receipt_page_got']))
				unset($_SESSION['novalnet']['novalnet_paypal_receipt_page_got']);
			if(isset($_SESSION['novalnet']['novalnet_paypal']['paypal_info_got']))
				unset($_SESSION['novalnet']['novalnet_paypal']['paypal_info_got']);
		...
		function init_form_fields() {
			...
			$this->form_fields = array(
				...
				'paypal_pending_status' 	=> array(
					'title' 	=> __( 'PayPal payment pending status', 'novalnet' ),
					'type' 		=> 'select',
		...
		function receipt_page( $order_id ) {
			if (!isset($_SESSION['novalnet']['novalnet_paypal_receipt_page_got'])) {
				...
				echo $this->get_novalnet_paypal_form_html($order_id);
				$_SESSION['novalnet']['novalnet_paypal_receipt_page_got'] = 1;
			}
		}
		 ...
		 */
		public function get_novalnet_paypal_form_html($order_id) {
			global $woocommerce;
			$order = new WC_Order( $order_id );
			...
			$script = '
				...
				jQuery("#submit_novalnet_paypal_payment_form").click();
			';
			...
			return '<form id="frmnovalnet" name="frmnovalnet" action="' . (is_ssl() ? 'https://' : 'http://').PAYPAL_PAYPORT_URL . '" method="post" target="_top">' . implode('', $novalnet_args_array) . '
			<input type="submit" class="button-alt" id="submit_novalnet_paypal_payment_form" value="' . __('Pay via Novalnet', 'novalnet') . '" /> <a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . __('Cancel order &amp; restore cart', 'novalnet') . '</a>
		</form>';
		}   // End get_novalnet_form_html()

		function check_paypal_response(){
			if(isset($_REQUEST['status']) && isset( $_REQUEST['user_variable_0'] ) && $_REQUEST['payment_type'] == "PAYPAL" && isset($_REQUEST['input1'])){
		...
		public function novalnet_paypal_transactional_info($order) {
			if (!isset($_SESSION['novalnet']['novalnet_paypal']['paypal_info_got']) && $order->payment_method == $this->id) {
				echo wpautop('<h2>' . __('Transaction Information', 'novalnet') . '</h2>');
				echo wpautop(wptexturize($order->customer_note));
			   $_SESSION['novalnet']['novalnet_paypal']['paypal_info_got'] = 1;
			}
		}	// End novalnet_paypal_transactional_info()
