<?php
/**
 * Sofortüberweisung / Instant bank transfer Payment Gateway by Novalnet
 *
 ...
 *
 * @class       WC_Gateway_Novalnet_Banktransfer
 * @extends     WC_Novalnet_Payment_Gateway
 * @version     2.0.0
 * @package     woocommerce-novalnet-gateway/includes/gateways/novalnet_banktransfer
 * @author      Novalnet
 ...
 */
 ...
 /*** Install Sofortüberweisung / Instant bank transfer payment to Novalnet Payment Gateway ***/
 ...
 function init_gateway_novalnet_banktransfer() {
	...
	class WC_Gateway_Novalnet_Banktransfer extends WC_Novalnet_Payment_Gateway {

		var $id = NOVALNET_BT;
		var $payment_key = PAYMENT_KEY_BT;
		var $payment_type = 'ONLINE_TRANSFER';
		...
		public function __construct() {
			...
			$this->init_settings();
			$this->method_title     = 'Novalnet ' . __( 'Instant Bank Transfer', 'novalnet' );
			...
			$this->icon             = NOVALNET_URL . 'assets/images/novalnet_banktransfer.png';
			...
			add_action( 'init', array( &$this, 'check_sofort_response' ) );
			...
			if ( isset( $_SESSION['novalnet']['novalnet_banktransfer']['banktransfer_info_got'] ) )
				unset( $_SESSION['novalnet']['novalnet_banktransfer']['banktransfer_info_got'] );

			if ( isset( $_SESSION['novalnet']['novalnet_banktransfer_receipt_page_got'] ) )
				unset( $_SESSION['novalnet']['novalnet_banktransfer_receipt_page_got'] );
		...
		function receipt_page( $order_id ) {
			if ( !isset( $_SESSION['novalnet']['novalnet_banktransfer_receipt_page_got'] ) ) {
				...
				echo $this->get_novalnet_banktransfer_form_html( $order_id );
				$_SESSION['novalnet']['novalnet_banktransfer_receipt_page_got'] = 1;
			}
		}

		public function get_novalnet_banktransfer_form_html( $order_id ) {
			global $woocommerce;
			$order              = new WC_Order( $order_id );
			...
			$script = '
				...
				jQuery("#submit_novalnet_sofort_payment_form").click();
			';
			...
			<input type="submit" class="button-alt" id="submit_novalnet_sofort_payment_form" value="' . __( 'Pay via Novalnet', 'novalnet' ) . '" /> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . __( 'Cancel order &amp; restore cart', 'novalnet' ) . '</a>
		</form>';
		}

		function check_sofort_response() {

			if ( isset( $_REQUEST['status'] ) && isset( $_REQUEST['user_variable_0'] ) &&isset( $_REQUEST['payment_type'] ) && $_REQUEST['payment_type'] == "ONLINE_TRANSFER" ) {
		...
		public function novalnet_banktransfer_transactional_info( $order ) {

			if ( !isset( $_SESSION['novalnet']['novalnet_banktransfer']['banktransfer_info_got'] ) && $order->payment_method == $this->id ) {
				echo wpautop( '<h2>' . __( 'Transaction Information', 'novalnet' ) . '</h2>' );
				echo wpautop( wptexturize( $order->customer_note ) );
				$_SESSION['novalnet']['novalnet_banktransfer']['banktransfer_info_got'] = 1;
			}
		}
	} // End class NOVALNET_BT_CLASS

	$obj = new WC_Gateway_Novalnet_Banktransfer();
