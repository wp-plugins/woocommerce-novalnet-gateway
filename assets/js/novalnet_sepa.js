  ...
  jQuery( function( $ ) {
	$( document ).ready(function() {
        ...
        if ( version == '2.1.3' || version == '2.1.7' ) {
			$("body").on( "click", "#novalnet_sepa_mandate_confirm", function() {
				perform_sepa_iban_bic_call();
			});
		} else {
			$( '#novalnet_sepa_mandate_confirm' ).on( 'click', function () {
				perform_sepa_iban_bic_call();
			} );
		}
		...
		function show_mandate_overlay() {
			...
			if ( $( '#novalnet_customer_info' ).length && $( '#novalnet_customer_info' ).val() != '' ) {
				...
				$( '#sepa_overlay_enduser_country_span' ).html( $( '#novalnet_sepa_bank_country' ).val() );
			...
			if ( response.hash_result == 'success' ) {
				switch( code ){
					...
					case 'separefill':
						...
						$( '#novalnet_sepa_account_holder' ).val( decodeURIComponent( account_holder ) );
